...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _submit() async {
    ...
    if (!email.contains('@') || !email.contains('.')) { _showSnack('Format email tidak valid'); return; }
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                if (mounted) _showSnack(ok ? '✅ Email konfirmasi dikirim ulang!' : '❌ Gagal');
              },
              child: const Text('Kirim Ulang')),
          ],
        ));
      } else {
        _showSnack(err.isNotEmpty ? err : (_isRegister ? 'Gagal daftar.' : 'Email atau password salah.'), isError: true);
      }
    }
  }
  ...
  Future<void> _forgotPassword() async {
    ...
    await showDialog(context: context, builder: (ctx) => AlertDialog(
      ...
      content: Column(mainAxisSize: MainAxisSize.min, children: [
        const Text('Masukkan email untuk link reset password.',
      ...
      actions: [
        ...
        ElevatedButton(onPressed: () async {
          ...
          if (mounted) _showSnack(ok ? '✅ Link reset dikirim ke $email' : '❌ Gagal kirim email');
      ...
      body: Column(children: [
        Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text(_isRegister ? 'Buat Akun Baru' : 'Selamat datang di\nBapander',
              style: const TextStyle(fontSize: 24, fontWeight: FontWeight.w700, color: Colors.white, height: 1.3)),
            const SizedBox(height: 6),
            Text(_isRegister ? 'Daftar untuk mulai mengobrol' : 'Chat komunitas dengan bahasa daerahmu',
          ...
          child: Column(children: [
            ...
            TextField(controller: _passCtrl, obscureText: _obscure,
              decoration: InputDecoration(
                hintText: 'Password (min. 6 karakter)', prefixIcon: const Icon(Icons.lock_outline),
